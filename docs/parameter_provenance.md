...
Run `python3 utils/eoh_cli.py provenance check` for the live figures. At the time of
the split, over all 229 constants:
...
| **placeholder** | **106** | **46.3%** | **no measurement behind it at all — the debt** |
| normative | 60 | 26.2% | a decision; no dataset retires it |

**Debt is 52.4%, not 83.3%** — and the actionable figure is the 46.3% of constants
that are placeholders. That is the number to drive down. The 60 normative constants
are not debt; they are what the framework has decided, and they need argument rather
than data.
...
| `SKILL_WORKING_LIFE_YEARS` | 40.0 | years, entry to retirement | placeholder | BLS Employee Tenure, or cohort exit rates from the labour force.<br>the only input to transmission, and weakly held — but halving or doubling it moves transmission 2×, against ε_ref's 7.13× lever on KNOWLEDGE_EOH_BASE, so it is not where the uncertainty lives. |
...
| `DEFAULT_SEGMENTS` | <list: 4 items> | fractions of workforce and dimensionless multipliers | placeholder | SUPERSEDED IN PRACTICE. reference/workforce.py ships measured workforce composition snapshots and scenarios/measured.py builds segments from the O*NET/BLS registry (751 occupations, 94.2% of US employment). This default survives as a self-contained fixture for tests and examples; prefer the measured path for any reported result.<br>CALIBRATED TO A TARGET — the segment means were set so the weighted mean lands on 2.10, the top of the constitutional band, at ε=0. Same class as the GUF_USE_* rates: a value reverse-engineered from a desired outcome. |
<!-- /provenance:table -->
...
| `CONTESTABILITY_PHI_FLOOR` | 0.1 | fraction of automation value held in common | placeholder<br>form: φ(0) — even at subsistence some automation value is commonly held (the Trust baseline). | n/a in the adopted model — §8.9b makes φ(ε) emerge from the charter formation share under a stated policy (dilution / target / escalated) rather than from a floor plus a power law. Kept for the superseded arm. |
| `CONTESTABILITY_PHI_EXPONENT` | 1.5 | dimensionless power | placeholder<br>form: sub-linear growth of commonization early in the arc (ε^1.5 rather than ε), asserting that political-economy constraints make rapid commonization hard. | n/a — superseded by the charter-formation model, as above. |
| `CONTESTABILITY_G_PRIV` | 0.03 | fraction per year | placeholder<br>form: g_priv, the private capital growth rate. The Piketty-inversion condition requires dτ/dε ≥ 0, i.e. the Trust must grow faster than private capital. | real capital returns net of depreciation for the jurisdiction being modelled; Piketty's r series is the standard source and gives 4–5% historically.<br>at canonical defaults the levy-alone path to that condition is infeasible (levy_fraction ≫ 1) — the adversarial finding of reconciliation §8.3, and §8.9 showed the failure was the miscalibrated cash-Trust frame rather than the levy. §8.9c then found endogenous g_priv turns NEGATIVE past ε≈0.5, so this fixed 3% is not the operative reading in the adopted model. |
| `CONTESTABILITY_CAPITAL_YIELD_RATE` | 0.1 | fraction per year | placeholder<br>form: the annual yield on automated capital, used to compute automated_output_teh = ε × capital_stock × yield. | 1/RECAL_CAPITAL_OUTPUT_RATIO − FORMATION_DEPRECIATION_RATE = 0.20 is the same quantity derived elsewhere in this file, against 0.10 here. A 2× disagreement between two capital-yield constants in one repo; they should be reconciled to one derivation. |
...
| `ETA_LAND_MASK_THRESHOLD` | 0.5 | ERA5 land-sea-mask fraction ∈ [0,1] | normative<br>form: lsm ≥ this counts as land (§5 decision 1: territorial sea excluded). The ERA5 mask is a fraction, so a threshold is required; 0.50 is the natural midpoint but it IS a threshold on a continuous field, not a measurement. | **decided by** a governance decision on whether partly-marine cells bear an allocation. The underlying field is measured (ERA5); where the line falls is not.<br>_no measurement settles this_ |
