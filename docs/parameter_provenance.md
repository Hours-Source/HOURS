...
Canonical-arc figures, re-measured 2026-09-01 from
`python3 utils/eoh_cli.py --no-color arc --domain-shares --points 100`:

| Domain | ε = 0 | ε = 0.40 | ε = 0.99 |
|---|---|---|---|
| personal | 99.1% | 85.8% | **48.3%** |
| infrastructure | 0.0% | 5.3% | 8.0% |
| knowledge | 0.9% | 9.0% | 43.8% |
...
48.3% against knowledge 43.8%. Two changes on 2026-09-01 did it, and both moved
the same way: `AGE_WEIGHT_CHILD` took the MTUS self-maintenance measurement for
ages 6–14 (1.5 → 1.82), which raises personal; and the ε_ref fixed point
re-anchored `KNOWLEDGE_EOH_BASE` **−9.94%** with it, which lowers knowledge.*
...
to do with the knowledge domain. **The anchor has now moved seven times and the
ordering has flipped back.** The caution stands, unchanged and now demonstrated:
