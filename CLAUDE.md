...
**2420 tests passing, mypy clean on 67 source files** (2026-08-08). Workstreams A–F merged to main, including the contestability closure (derived levy schedule, marginal χ, dashboard wiring) and Coasean Phase 3 (trust dynamics, settlement rules, desire stub).
...
- **`reference/personal_basket.py`** — basket in physical units. **One component of six is priced**: nutrition production, 767,025 kcal/yr ÷ 2,317.8 kcal per labour-hour (LSMS-ISA, 7 countries, unassisted stratum) = **330.9 h/person·yr**, with an independent cross-check 7.6% away. Everything else declares `hours_per_unit=None`. `test_only_nutrition_production_is_priced` guards the discipline: an invented productivity would enter with the same standing as the measured one and afterwards nothing could tell them apart.
...
- **D — the §3.3 conservation test does not show elimination (first pass).** H_food_total(US) ≈ **392.0 h/person·yr** (SOC 35+45+51-3 employment 86.0, ATUS 0202 215.6, 0701 90.4) vs LSMS ε≈0.05 crop production alone **320**. Higher, not "substantially lower" — automation RELOCATED food labour between ledgers. Both sides undercounted, baskets not held fixed. **Scratch-script numbers; reproduce in `scenarios/` before citing.**
...
- **E — the ε_ref anchor is not a fixed point.** Labour-residual now gives **0.470**, not 0.391: `KNOWLEDGE_EOH_BASE` was derived at ε_ref=0.40 on a corroboration that K-IV's own +12% mid-arc growth then consumed. Base stays frozen; re-derivation is a sign-off item and wants an iteration to the fixed point, not a second one-shot anchor.
