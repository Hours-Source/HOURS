...
- **KNOWN LIMIT, stated not implied**: the tuple form is accepted on its literal label and the check does not follow a loop variable bound from it (`doctrine_arc`'s `("shipped", …)` row feeds a computed series — legitimately, since that series IS the refuted doctrine). Following the taint needs dataflow analysis; `test_the_known_limit_is_the_documented_one` fails if the docstring stops saying so.
