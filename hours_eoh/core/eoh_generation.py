...
def ecological_eoh(
    ...
    accumulation are punctuated by sharp threshold failures. Below the health
    threshold, a spike function captures the nonlinear compounding of neglect
    (fishery collapse, soil depletion, aquifer loss, climate tipping points).

    Historical neglect means deferred EOH has been accumulating unaccounted.
    The framework makes this obligation visible (Guardrail I: physical grounding).
    Better monitoring technology makes more deferred obligations legible.

    **Physical state input**: monitoring_capability is the explicit physical
    parameter — the fraction of deferred ecological EOH that the civilization's
    monitoring systems can detect and report, ∈ [0.0, 1.0]. At ε=0 (subsistence),
    monitoring is minimal; as automated sensing and ecological science develop,
    monitoring capability grows. Pass it directly from the simulation state.

    **Backward compatibility**: If epsilon is provided and monitoring_capability
    is None, the canonical monitoring capability for that ε is used:
    CANONICAL_MONITORING_CAPABILITY_BASE + CANONICAL_MONITORING_CAPABILITY_SLOPE × ε.
