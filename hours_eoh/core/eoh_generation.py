...
def resolve_capital_stock(
    ...
    **THE FRAME (2026-09-16).** `CAPITAL_STOCK_DEFAULT` is declared "at the 1M
    reference population", and until now this resolver had no way to hear about
    any other one: it returned the same absolute stock whatever population the
    caller was modelling. 25 of the 32 functions that call it have a
    `population` in scope, and exactly one of them scaled. The result at
    `min_levy_for_solvency`, capital not supplied: stewardship per capita
    1,078.07 / 107.81 / 10.78 TEH at populations 1e5 / 1e6 / 1e7 — exactly
    inverse-proportional, while the guarantee per capita stays 139.85. A
    collective of 100,000 was told it owed ten times the stewardship labour per
    person as the reference frame, for identical capital INTENSITY.

    `population` resolves that: the frame holds capital intensity fixed, so a
    resolved stock scales by `population / REFERENCE_FRAME_POPULATION`. Omitting
    it keeps the reference frame, so this is additive — every existing caller
    reads exactly what it read before, and the ones that mix frames are wired
    deliberately rather than silently corrected.

    **A SUPPLIED STOCK IS STILL NEVER RESCALED**, by population any more than by
    ε. A caller that names a stock is naming the actual one, and scaling it
    would destroy their input — the same rule the ε branch already follows.
    """
    if capital_stock is not None:
        # Never rescaled — by population or by ε. See the docstring.
        return capital_stock
    # A RESOLVED stock is the reference frame's. Scale it to the caller's
    # population, holding capital INTENSITY fixed rather than absolute stock.
    # `population=None` keeps the reference frame, which is what every caller
    # that has not been wired yet still gets.
    scale = 1.0 if population is None else population / REFERENCE_FRAME_POPULATION
    if epsilon is not None:
        return (CAPITAL_STOCK_DEFAULT * (1.0 + CANONICAL_CAPITAL_GROWTH_SLOPE)
                * epsilon * scale)
    return CAPITAL_STOCK_DEFAULT * scale
...
def infrastructure_eoh(
    ...
    **A SUPPLIED STOCK IS NEVER RESCALED** (2026-09-09, author decision). ε fills
    ...
    Args:
        ...
        population: Optional. THE FRAME (2026-09-16). Like epsilon, it applies
                 ONLY to a capital_stock that is None: a RESOLVED stock belongs
                 to the 1M reference population and is scaled to this one,
                 holding capital intensity fixed. A supplied stock is the actual
                 stock and is never rescaled. Omitted keeps the reference frame.
                 This is the path the documented entry point takes —
                 eoh_to_teh_pipeline → total_eoh → here — so omitting it was why
                 per-capita output ran 913.65 / 339.64 / 282.23 TEH at
                 populations 1e5 / 1e6 / 1e7 for identical capital intensity.
    ...
    effective_capital = resolve_capital_stock(capital_stock, epsilon, population)
...
def knowledge_eoh(
    ...
    At high automation, much of the remaining human contribution is knowledge
    maintenance, transmission, and judgment. Skills atrophy. Institutional
    memory fades. Software rots. Standards drift. This domain grows with ε —
    to roughly a third of the obligation at ε = 0.99 on the canonical arc,
    while personal EOH stays the largest domain — and is the hardest to verify, requiring careful consideration
...
def total_eoh(
    ...
    **Physical state design**: When epsilon is None, each domain function uses
    ...
    # THE FRAME. `population` reaches infrastructure_eoh for the same reason it
    # reaches the ecological term below: a capital stock this function RESOLVES
    # (rather than one the caller supplies) is stated at the 1M reference, and
    # summing it against a personal term already scaled by population put the
    # two domains on different footings. Same defect as the ecological one Phase
    # 4b closed, one domain over.
    # BOUND, not copied (2026-09-16): this passed a bare `2.0` for
    # `age_factor_max`, whose own default IS `INFRA_AGE_FACTOR_MAX = 2.0` —
